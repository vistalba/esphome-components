...
struct MeterInfo {
  std::string bus; // The bus used to communicate with this meter. A device like
                   // /dev/ttyUSB0 or an alias like BUS1. A bus can be an mbus
                   // or a wmbus dongle. The bus can be the empty string, which
                   // means that it will fallback to the first defined bus.
  ...
  std::vector<AddressExpression>
      address_expressions;      // Match expressions for ids.
  IdentityMode identity_mode{}; // How to group telegram content into objects
                                // with state. Default is by id.
  ...
  std::vector<std::string>
      extra_constant_fields; // Additional static fields that are added to each
                             // message.
  std::vector<std::string>
      extra_calculated_fields; // Additional field calculated using formulas.
  std::vector<std::string>
  ...
  bool parse(std::string name, std::string driver, std::string id,
             std::string key);
  bool usesPolling();
};
...
private:
  ...
  std::vector<DriverName>
      name_aliases_;      // Secondary names that will map to this driver.
  LinkModeSet linkmodes_; // C1, T1, S1 or combinations thereof.
  Translate::Lookup
      mfct_tpl_status_bits_; // Translate any mfct specific bits in tpl status.
  MeterType type_;           // Water, Electricity etc.
  std::function<std::shared_ptr<Meter>(MeterInfo &, DriverInfo &di)>
  ...
  int force_mfct_index_ =
      -1; // Used for meters not declaring mfct specific data using the dif 0f.
  bool has_process_content_ =
      false; // Mark this driver as having mfct specific decoding.
...
public:
  ...
  void setDefaultFields(std::string f) {
    default_fields_ = splitString(f, ',');
  }
  void addLinkMode(LinkMode lm) { linkmodes_.addLinkMode(lm); }
  void forceMfctIndex(int i) { force_mfct_index_ = i; }
  void setConstructor(
      std::function<std::shared_ptr<Meter>(MeterInfo &, DriverInfo &)> c) {
    constructor_ = c;
  }
  void addDetection(uint16_t mfct, uchar type, uchar ver) {
    detect_.push_back({mfct, type, ver});
  }
  ...
  std::shared_ptr<Meter> construct(MeterInfo &mi) {
    return constructor_(mi, *this);
  }
...
enum PrintProperty {
  ...
  INJECT_INTO_STATUS = 16, // This text field is injected into the already
                           // defined status field. multiple OK:s collapse.
...
struct FieldInfo {
  ~FieldInfo();
  FieldInfo(int index, std::string vname, Quantity quantity, Unit display_unit,
            VifScaling vif_scaling, DifSignedness dif_signedness, double scale,
            FieldMatcher matcher, std::string help,
  ...
  // A FieldInfo can be declared to handle any number of storage fields of a
  // certain range. The vname is then a pattern total_at_month_{storage_counter}
  // that gets translated into total_at_month_2 (for the dventry with storage
  // nr 2.)
...
private:
  ...
  std::string vname_; // Value name, like: total current previous target, ie no
                      // unit suffix.
  ...
  double scale_; // A hardcoded scale factor. Used only for manufacturer
                 // specific values with unknown units for the vifs.
  ...
  // Normally the values are stored inside the meter object using its
  // setNumeric/setString/getNumeric/getString But for special fields we can
  // override this default location with these setters/getters.
  std::function<double(Unit)>
      get_numeric_value_override_; // Callback to fetch the value from the
                                   // meter.
  std::function<std::string()>
      get_string_value_override_; // Callback to fetch the value from the meter.
  std::function<void(Unit, double)>
      set_numeric_value_override_; // Call back to set the value in the c++
                                   // object
  std::function<void(std::string)>
      set_string_value_override_; // Call back to set the value string in the
                                  // c++ object
...
struct Meter {
  ...
  // Either the default fields specified in the driver, or override fields in
  // the meter configuration file.
  ...
  virtual void setNumericValue(FieldInfo *fi, DVEntry *dve, Unit u,
                               double v) = 0;
  ...
  virtual void setStringValue(std::string vname, std::string v,
                              DVEntry *dve = NULL) = 0;
  ...
  // The handleTelegram expects an input_frame where the DLL crcs have been
  // removed. Returns true of this meter handled this telegram! Sets id_match to
  // true, if there was an id match, even though the telegram could not be
  // properly handled.
  virtual bool handleTelegram(AboutTelegram &about,
                              std::vector<uchar> input_frame, bool simulated,
                              std::vector<Address> *addresses, bool *id_match,
                              Telegram *out_t = NULL) = 0;
